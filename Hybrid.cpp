...
void ResetMap::reset(TaylorModelVec & result, const TaylorModelVec & tmv, const vector<Interval> & domain, const int order, const Interval & cutoff_threshold) const
{
...
HybridSystem::HybridSystem(const HybridSystem & hybsys)
{
	...
	linear_invariants	=	hybsys.linear_invariants;
...
void HybridSystem::reach_continuous_linear(list<TaylorModelVec> & resultsCompo, list<vector<Interval> > & domains, const int mode, const Flowpipe & initFp,
		...
		const vector<string> & modeNames, const vector<string> & stateVarNames, const Interval & cutoff_threshold) const
{
	...
	int rangeDim = odes[mode].tms.size();
	...
	odes[mode].linearCoefficients(coefficients);
	...
	odes[mode].constant(constant_part);

	Interval_matrix imConstant(constant_part);
	Interval_matrix B = transConst * imConstant;

	vector<Interval> centered_B;
	for(int i=0; i<rangeDim; ++i)
	{
		Interval intTemp;
		B.get(intTemp, i, 0);
		intTemp.remove_midpoint();
		centered_B.push_back(intTemp);
	}
	...
	initFp.intEval(rangeOfx0, cutoff_threshold);
	...
	initFp.tmvPre.polyRangeNormal(polyRangeX0, step_exp_table);
	...
	for(int i=0; i<rangeDim; ++i)
	{
		...
		hf.insert_ctrunc_normal(tmTemp, initFp.tmvPre, polyRangeX0, step_exp_table, rangeDim+1, order, cutoff_threshold);
	...
	vector<Interval> newDomain = initFp.domain;
	...
	resultsCompo.push_back(initFp.tmvPre);
...
/*
	...
	if(linear_invariants[mode].constraints.size() > 0)
	{
		use_local_inv = true;
	}
	else
	...
	if(use_local_inv)
	{
		for(int i=0; i<constraint_template.size(); ++i)
		{
			constraint_template[i].B = linear_invariants[mode].rho(constraint_template[i].A);
		}
	}

	Polyhedron local_inv = linear_invariants[mode];

	if(use_local_inv)
	{
		for(int i=0; i<constraint_template.size(); ++i)
		{
			local_inv.constraints.push_back(constraint_template[i]);
		}
	}
*/

	vector<bool> local_boundary_intersected;
	int type = contract_interval_arithmetic(x, newDomain, linear_invariants[mode], local_boundary_intersected);
...
/*
	...
	{
		// collect the intersected invariant boundary
		if(invariant_boundary_intersected.size() != linear_invariants[mode].constraints.size())
		{
			for(int i=0; i<linear_invariants[mode].constraints.size(); ++i)
			{
				invariant_boundary_intersected.push_back(local_boundary_intersected[i]);
			}
		}
		else
		{
			for(int i=0; i<invariant_boundary_intersected.size(); ++i)
			{
				if(local_boundary_intersected[i])
				{
					invariant_boundary_intersected[i] = true;
...
*/
	...
	case 2: 	// time interval is contracted
		if(newDomain[0] > intZero)
		{
			return;
		}
		else
		{
			resultsCompo.push_back(x);
...
//	int add_start = linear_invariants[mode].constraints.size();

	vector<Polynomial> P0;
	vector<Interval> R0;
	for(int i=0; i<rangeDim; ++i)
	{
		P0.push_back(x.tms[i].expansion);
		R0.push_back(x.tms[i].remainder);
	}
...
/*
			if(use_local_inv)
			{
				intSup = linear_invariants[mode].rho(vec_Vn);
				bounds_In.push_back(intSup);
			}
*/
		}

		// the first way to compute the remainder part
		for(int i=0, j=rangeDim; i<rangeDim; ++i,++j)
		{
			...
			newFlowpipe.tms.push_back(tmTemp);
		}

/*
		if(use_local_inv)
		{
			// compute the constraints generated from the mode invariant
			for(int i=0; i<constraint_template.size(); ++i)
			{
				Interval intTemp = bounds_In[i] + bounds_Vn[i];

				if(constraint_template[i].B > intTemp)
				{
					constraint_template[i].B = intTemp;
				}
			}

			// update the local invariant
			for(int i=0; i<constraint_template.size(); ++i)
			{
				local_inv.constraints[i+add_start].B = constraint_template[i].B;
			}
		}
*/

		// the second way to compute the remainder part
		Interval_matrix remVector(rangeDim, 1);
		for(int i=0; i<rangeDim; ++i)
		{
			remVector.set(currentFlowpipe.tms[i].remainder, i, 0);
		}

		remVector = expAr * remVector;

		// make an intersection
		for(int i=0; i<rangeDim; ++i)
		{
			Interval intTemp;
			remVector.get(intTemp, i, 0);

			intTemp += centered_B[i];
			newFlowpipe.tms[i].remainder.intersect_assign(intTemp);
		}

		newFlowpipe.cutoff_normal(step_exp_table, cutoff_threshold);

		int type = contract_interval_arithmetic(newFlowpipe, newDomain, linear_invariants[mode], local_boundary_intersected);
...
*/
		...
		case 2: 	// time interval is contracted
			...
			{
				resultsCompo.push_back(newFlowpipe);
				domains.push_back(newDomain);
...
bool HybridSystem::reach_continuous_picard(list<TaylorModelVec> & resultsCompo, list<vector<Interval> > & domains, const int mode, const Flowpipe & initFp,
		...
		const vector<string> & stateVarNames, vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	initFp.composition_normal(tmvTemp, step_exp_table, cutoff_threshold);
	...
	{
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrder, maxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		...
		int res = currentFlowpipe.advance_picard(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrders, localMaxOrder, maxOrders, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], taylorExpansion, precondition, step_exp_table, step_end_exp_table, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], taylorExpansion, precondition, step_exp_table, step_end_exp_table, orders, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], taylorExpansion, precondition, step_exp_table, step_end_exp_table, newStep, miniStep, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], taylorExpansion, precondition, step_exp_table, step_end_exp_table, newStep, miniStep, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
...
bool HybridSystem::reach_continuous_low_degree(list<TaylorModelVec> & resultsCompo, list<vector<Interval> > & domains, const int mode, const Flowpipe & initFp,
		const double step, const double time, const int order, const int maxOrder, const int precondition, const vector<Interval> & estimation,
		const bool bPrint, const vector<string> & stateVarNames, vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	vector<Interval> step_exp_table, step_end_exp_table;
	Interval intZero;
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], expansions[newOrder-order], precondition, step_exp_table, step_end_exp_table, newOrder, maxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		...
		int res = currentFlowpipe.advance_low_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], taylorExpansionHF, precondition, step_exp_table, step_end_exp_table, newOrders, maxOrders, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		const bool bPrint, const vector<string> & stateVarNames, vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrder, maxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		...
		int res = currentFlowpipe.advance_high_degree(newFlowpipe, hfOdes[mode], hfOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrders, localMaxOrder, maxOrders, estimation, invariants[mode], cutoff_threshold);
		...
		const vector<string> & stateVarNames, vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, order, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newStep, miniStep, orders, globalMaxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrder, maxOrder, estimation, invariants[mode], cutoff_threshold);
		...
		vector<bool> & invariant_boundary_intersected, const vector<string> & modeNames, const Interval & cutoff_threshold) const
{
	...
	for(double t=THRESHOLD_HIGH; t < time;)
	{
		...
		int res = currentFlowpipe.advance_non_polynomial_taylor(newFlowpipe, strOdes[mode], strOdes_centered[mode], precondition, step_exp_table, step_end_exp_table, newOrders, localMaxOrder, maxOrders, estimation, invariants[mode], cutoff_threshold);
...
void HybridSystem::reach_hybrid(list<list<TaylorModelVec> > & resultsCompo, list<list<vector<Interval> > > & domains, list<int> & modeIDs, list<TreeNode *> & traceNodes,
		TreeNode * & traceTree, const vector<int> & integrationSchemes,	const double time, const int maxJmps, const ReachabilitySetting & global_setting, const vector<ReachabilitySetting> & local_settings,
		...
		const bool bPrint, const vector<string> & stateVarNames, const vector<string> & modeNames, const vector<string> & tmVarNames) const
{
	...
	for(; modeQueue.size() != 0;)
	{
		...
		const vector<int> *porders;
		const vector<int> *pmaxOrders;
		int globalMaxOrder;
		...
		const vector<Interval> *pestimation;
		...
		case ONLY_PICARD:
		{
			...
			case UNIFORM:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], (*pmaxOrders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			case MULTI:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, *pmaxOrders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_picard(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			}
			break;
		...
		case LOW_DEGREE:
		{
			...
			case UNIFORM:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], (*pmaxOrders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			case MULTI:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, *pmaxOrders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_low_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			}
			break;
		...
		case HIGH_DEGREE:
		{
			...
			case UNIFORM:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], (*pmaxOrders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			case MULTI:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, *pmaxOrders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_high_degree(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			}
			break;
		...
		case NONPOLY_TAYLOR:
		{
			...
			case UNIFORM:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], (*pmaxOrders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			case MULTI:
				if(bAdaptiveSteps)
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, miniStep, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else if(bAdaptiveOrders)
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, *pmaxOrders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				else
				{
					bvalid = reach_continuous_non_polynomial_taylor(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, *porders, globalMaxOrder, precondition, *pestimation, bPrint, stateVarNames, invariant_boundary_intersected, modeNames, cutoff_threshold);
				}
				break;
			}
			break;
		...
		{
			bvalid = true;
			reach_continuous_linear(mode_flowpipes, mode_domains, initMode, initFp, step, time-timePassed, (*porders)[0], bPrint, invariant_boundary_intersected, modeNames, stateVarNames, cutoff_threshold);
			break;
		}
		}
...
//	build_constraint_template((int)(stateVarNames.size()));
	...
	system.reach_hybrid(flowpipesCompo, domains, modeIDs, traceNodes, traceTree, integrationSchemes, time, maxJumps,
...
bool HybridReachability::declarePar(const string & pName, const Interval & range)
{
	map<string,int>::const_iterator iter;
...
int HybridReachability::getIDForPar(const string & pName) const
{
	map<string,int>::const_iterator iter;
	if((iter = parTab.find(pName)) == parTab.end())
	{
...
bool HybridReachability::getParName(string & pName, int id) const
{
...
bool HybridReachability::getRangeForPar(Interval & range, const string & pName) const
{
...
bool HybridReachability::declareMode(const string & mName, const TaylorModelVec & ode, const vector<PolynomialConstraint> & inv, const int integrationScheme, const ReachabilitySetting & local_setting)
{
	map<string,int>::const_iterator iter;
	Interval intZero;
...
/*
		if(modeID > system.odes.size())
		{
			for(int i=system.odes.size(); i<modeID; ++i)
			{
				...
				system.odes_centered.push_back(tmvEmpty);
				system.hfOdes_centered.push_back(hfsEmpty);
			}
		}
*/
		...
		if(integrationScheme == LINEAR)
		{
			...
			{
				Polynomial polyTemp = inv[i].p;

				Interval intTemp;
				polyTemp.constant(intTemp);
				polyTemp.rmConstant();

				intTemp = inv[i].B - intTemp;
				...
				polyTemp.constraintCoefficients(A);

				LinearConstraint lc(A, intTemp);
				lcs.push_back(lc);
			}

			Polyhedron polyhedron(lcs);
			system.linear_invariants.push_back(polyhedron);
		}
		else
		{
			Polyhedron empty_polyhedron;
			system.linear_invariants.push_back(empty_polyhedron);
		}

		// form an empty non-polynomial ODE
...
bool HybridReachability::declareMode(const string & mName, const vector<string> & strOde, const vector<PolynomialConstraint> & inv, const int integrationScheme, const ReachabilitySetting & local_setting)
{
	map<string,int>::const_iterator iter;
	Interval intZero;
...
*/
		...
		// put empty ODEs to the vectors for polynomial ODEs
		TaylorModelVec odeEmpty;
		vector<HornerForm> hfOdeEmpty;
...
void HybridReachability::declareTrans(const int start, const int end, const vector<PolynomialConstraint> & guard, const ResetMap & reset, const int aggregType, const vector<vector<double> > & candidates)
{
	...
	case INTERVAL_AGGREG:
	{
		...
		vector<RowVector> empty_template;
...
void HybridReachability::constructWeightTab()
{
	...
	vector<bool> linear_inv_empty;
	vector<RowVector> template_inv_empty;
	Matrix emptyMat(1);
...
int HybridReachability::safetyChecking()
{
	...
	for(; fpIter!=flowpipesCompo.end(); ++fpIter, ++fpdoIter, ++modeIter, ++nodeIter)
	{
		...
		for(; tmvIter!=fpIter->end(); ++tmvIter, ++doIter)
		{
			...
			vector<Interval> tmvPolyRange;
			tmvIter->polyRange(tmvPolyRange, *doIter);
			...
			for(int i=0; i<unsafeSet[*modeIter].size(); ++i)
			{
				...
				// interval evaluation on the constraint
				unsafeSet[*modeIter][i].hf.insert(tmTemp, *tmvIter, tmvPolyRange, *doIter, cutoff_threshold);

				Interval intTemp;
				tmTemp.intEval(intTemp, *doIter);
...
*/
		...
		if(template_auto.size() > 0)
		{
			...
			{
				FactorTab vector_selected;
				bool bselected = vector_selection(vector_selected, lst_unselected, paraTemplate, template_auto, num_selected);
